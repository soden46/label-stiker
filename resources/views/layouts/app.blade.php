...
<div class="app-shell">
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="nav-list" aria-label="Navigasi utama">
            ...
            @if(auth()->user()->canAccess('labels.create'))<a href="{{ route('labels.create') }}" class="nav-item {{ request()->routeIs('labels.create', 'labels.show') ? 'active' : '' }}"><span class="nav-icon">▣</span> Cetak label</a>@endif
            ...
            @if(auth()->user()->isSuperAdmin())
                ...
                <a href="{{ route('users.index') }}" class="nav-item {{ request()->routeIs('users.*') ? 'active' : '' }}"><span class="nav-icon">U</span> Pengguna</a>
                <a href="{{ route('roles.index') }}" class="nav-item {{ request()->routeIs('roles.*') ? 'active' : '' }}"><span class="nav-icon">R</span> Role & akses</a>
            @endif
            ...
            @if(auth()->user()->canAccess('manufacturing.view'))<span class="nav-item nav-disabled"><span class="nav-icon">M</span> Produksi / MRP <em>soon</em></span>@endif
        </nav>
